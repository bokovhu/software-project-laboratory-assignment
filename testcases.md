A nyelv definíciója itt van: https://drive.google.com/drive/u/0/folders/1mcYym1SzXJYcjwaNAyH9Y4dG5J-te5zM

A prototipus tesztesetei:
1. Create Tiles [x]
2. Create Items [x]
3. Create Animals [x]
4. Move orangutan to safe tile [x]
5. Move panda to safe tile [x]
6. Move orangutan to broken tile [x]
7. Move panda to broken tile alone [x]
8. Move panda to broken tile in line [x]
9. Move orangutan to exit tile alone [x]
10. Move orangutan to exit tile with pandas [x]
11. Orangutan uses wardrobe alone [x]
12. Orangutan uses wardrobe with pandas [x]
13. Panda uses wardrobe [ ]
14. Panda sleeps alone [ ]
15. Panda sleeps in line [ ]
16. Panda gets scared alone [ ] 
17. Panda gets scared in line [ ] 
18. Panda jumps alone on safe tile [ ]
19. Panda jumps in line on safe tile [ ]
20. Panda jumps alone on fragile tile [ ]
21. Panda jumps in line on fragile tile [ ]
22. Panda jumps alone on broken tile [ ]
23. Panda jumps in line on broken tile [ ]
24. Orangutan lets go [ ]
25. Orangutan steals from Orangutan [ ]


## 1. Create Tiles

#### Bemenet:

`ADD FRAGILE TILE WITH ID 1;`
`ADD EXIT TILE WITH ID 2;`
`ADD START TILE WITH ID 3;`

`CONNECT TILE 1 WITH TILE 2;`
`CONNECT TILE 2 WITH TILE 3;`
`CONNECT TILE 1 WITH TILE 3;`

`/print`

#### Elvárt kimenet:

[TILES]

`FRAGILE TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`EXIT TILE ID = 2 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`START TILE ID = 3 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2`
`TILE 2 CONNECTED WITH TILE 3`
`TILE 1 CONNECTED WITH TILE 3`

## 2. Create items

#### Bemenet:

`ADD TILE WITH ID 1;`
`ADD TILE WITH ID 2;`
`ADD VENDINGMACHINE WITH ID 3 ONTO TILE 1;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (VENDINGMACHINE 3)`
`TILE ID = 2 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`

[ITEMS]

`WARDROBE ID = 4 PLACEDON = (TILE 2)`
`VENDINGMACHINE ID = 3 PLACEDON = (TILE 1)`

## 3. Create animals

#### Bemenet:

`ADD TILE WITH ID 1;`
`ADD TILE WITH ID 2;`
`ADD COWARD PANDA WITH ID 3 ONTO TILE 1;`
`ADD ORANGUTAN WITH ID 4 ONTO TILE 2;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (COWARD PANDA 3) CURRENTITEM = (NULL)`
`TILE ID = 2 CURRENTANIMAL = (ORANGUTAN 4) CURRENTITEM = (NULL)`

[ANIMALS]

`COWARD PANDA ID = 3 STANDINGON = (TILE 1) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`
`ORANGUTAN ID = 4 STANDINGON = (TILE 2) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL) THEFTTIMER = 0`

## 4. Move orangutan on safe tile

#### Bemenet:

`ADD TILE WITH ID 1;`
`ADD TILE WITH ID 2;`
`ADD ORANGUTAN WITH ID 3 ONTO TILE 1;`

`CONNECT TILE 1 WITH TILE 2;`

`BEGIN;`
`MOVE ORANGUTAN WITH ID 3 ONTO TILE 2;` 
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`TILE ID = 2 CURRENTANIMAL = (ORANGUTAN 3) CURRENTITEM = (NULL)`

[ANIMALS]

`ORANGUTAN ID = 4 STANDINGON = (TILE 2) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL) THEFTTIMER = 0`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2`

## 5. Move panda on safe tile

#### Bemenet:

`ADD TILE WITH ID 1;`
`ADD TILE WITH ID 2;`
`ADD SLEEPY PANDA WITH ID 3 ONTO TILE 1;`

`CONNECT TILE 1 WITH TILE 2;`

`BEGIN;`
`MOVE SLEEPY PANDA WITH ID 3 ONTO TILE 2;` 
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`TILE ID = 2 CURRENTANIMAL = (SLEEPY PANDA 3) CURRENTITEM = (NULL)`

[ANIMALS]

`SLEEPY PANDA ID = 3 STANDINGON = (TILE 2) SLEEPINGIN = (NULL) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2`

## 6. Move orangutan on broken tile

#### Bemenet:

`ADD TILE WITH ID 1;`
`ADD BROKEN TILE WITH ID 2;`
`ADD ORANGUTAN WITH ID 3 ONTO TILE 1;`

`CONNECT TILE 1 WITH TILE 2;`

`MOVE ORANGUTAN WITH ID 3 ONTO TILE 2;` 

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`BROKEN TILE ID = 2 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH BROKEN TILE 2`

## 7. Move panda on broken tile alone

#### Bemenet:

`ADD TILE WITH ID 1;`
`ADD BROKEN TILE WITH ID 2;`
`ADD SLEEPY PANDA WITH ID 3 ONTO TILE 1;`

`CONNECT TILE 1 WITH TILE 2;`

`BEGIN;`
`MOVE SLEEPY PANDA WITH ID 3 ONTO TILE 2;` 
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`BROKEN TILE ID = 2 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH BROKEN TILE 2`

## 8. Move panda on broken tile in line

#### Bemenet:

`/load panda_line_brokentile.txt`

`BEGIN;`
`MOVE ORANGUTAN WITH ID 5 ONTO TILE 4;`
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`TILE ID = 2 CURRENTANIMAL = (SLEEPY PANDA 5) CURRENTITEM = (NULL)`
`BROKEN TILE ID = 3 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`TILE ID = 4 CURRENTANIMAL = (ORANGUTAN 7) CURRENTITEM = (NULL)`

[ANIMALS]

`SLEEPY PANDA ID = 5 STANDINGON = (TILE 2) SLEEPINGIN = (NULL) LEADERANIMAL = (ORANGUTAN 7) GUIDEDANUMAL = (NULL)`
`ORANGUTAN ID = 7 STANDINGON = (TILE 4) LEADERANIMAL = (NULL) GUIDEDANUMAL = (SLEEPY PANDA 5) THEFTTIMER = 0`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2`
`TILE 2 CONNECTED WITH BROKEN TILE 3`
`BROKEN TILE 3 CONNECTED WITH TILE 4`

## 9. Move orangutan to exit tile alone

#### Bemenet:

`ADD ENTRANCE TILE WITH ID 1;`
`ADD EXIT TILE WITH ID 2;`
`ADD ORANGUTAN WITH ID 3 ONTO TILE 1;`

`CONNECT TILE 1 WITH TILE 2;`

`BEGIN;`
`MOVE ORANGUTAN WITH ID 3 ONTO TILE 2;` 
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`ENTRANCE TILE ID = 1 CURRENTANIMAL = (ORANGUTAN 3) CURRENTITEM = (NULL)`
`EXIT TILE ID = 2 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`

[ANIMALS]

`ORANGUTAN ID = 3 STANDINGON = (ENTRANCE TILE 1) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL) THEFTTIMER = 0`

[CONNECTIONS]

`ENTRANCE TILE 1 CONNECTED WITH EXIT TILE 2`

## 10. Move orangutan to exit tile with pandas

#### Bemenet:

`ADD ENTRANCE TILE WITH ID 1;`
`ADD TILE WITH ID 2;`
`ADD EXIT TILE WITH ID 3;`
`ADD ORANGUTAN WITH ID 4 ONTO TILE 2;`
`ADD SLEEPY PANDA WITH ID 5 ONTO TILE 1;`

`CONNECT TILE 1 WITH TILE 2;`
`CONNECT TILE 2 WITH TILE 3;`

`BEGIN;`
`MOVE ORANGUTAN WITH ID 4 ONTO TILE 1;`
`MOVE ORANGUTAN WITH ID 4 ONTO TILE 3;`
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`ENTRANCE TILE ID = 1 CURRENTANIMAL = (ORANGUTAN 4) CURRENTITEM = (NULL)`
`TILE ID = 2 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`EXIT TILE ID = 3 CURRENTANIMAL = (SLEEPY PANDA 5) CURRENTITEM = (NULL)`

[ANIMALS]

`ORANGUTAN ID = 4 STANDINGON = (ENTRANCE TILE 1) LEADERANIMAL = (NULL) GUIDEDANUMAL = (SLEEPY PANDA 5) THEFTTIMER = 0`
`SLEEPY PANDA ID = 5 STANDINGON = (EXIT TILE 3) SLEEPINGIN = (NULL) LEADERANIMAL = (ORANGUTAN 4) GUIDEDANUMAL = (NULL)`

[CONNECTIONS]

`ENTRANCE TILE 1 CONNECTED WITH TILE 2`
`TILE 2 CONNECTED WITH EXIT TILE 3`

## 11. Orangutan uses wardrobe alone

#### Bemenet:

`ADD TILE WITH ID 1;`
`ADD TILE WITH ID 2;`
`ADD TILE WITH ID 3;`
`ADD ORANGUTAN WITH ID 4 ONTO TILE 1;`
`ADD WARDROBE WITH ID 5 ONTO TILE 2;`
`ADD WARDROBE WITH ID 6 ONTO TILE 3;`

`CONNECT TILE 1 WITH TILE 2;`
`CONNECT WARDROBE 5 WITH WARDROBE 6;`

`BEGIN;`
`MOVE ORANGUTAN WITH ID 4 ONTO TILE 2;` 
`USE WARDROBE WITH ID 5 BY ORANGUTAN WITH ID 3;`
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`TILE ID = 2 CURRENTANIMAL = (NULL) CURRENTITEM = (WARDROBE 5)`
`TILE ID = 3 CURRENTANIMAL = (ORANGUTAN 4) CURRENTITEM = (WARDROBE 6)`

[ANIMALS]

`ORANGUTAN ID = 3 STANDINGON = (TILE 3) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL) THEFTTIMER = 0`

[ITEMS]

`WARDROBE ID = 5 PLACEDON = (TILE 2)`
`WARDROBE ID = 6 PLACEDON = (TILE 3)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2`
`WARDROBE 5 CONNECTED WITH WARDROBE 6`

## 12. Orangutan uses wardrobe with pandas

#### Bemenet:

`ADD TILE WITH ID 1;`
`ADD TILE WITH ID 2;`
`ADD TILE WITH ID 3;`
`ADD TILE WITH ID 4;`
`ADD ORANGUTAN WITH ID 5 ONTO TILE 2;`
`ADD SLEEPY PANDA WITH ID 6 ONTO TILE 1;`
`ADD WARDROBE WITH ID 7 ONTO TILE 3;`
`ADD WARDROBE WITH ID 8 ONTO TILE 4;`

`CONNECT TILE 1 WITH TILE 2;`
`CONNECT TILE 2 WITH TILE 3;`
`CONNECT WARDROBE 7 WITH WARDROBE 8`;

`BEGIN;`
`MOVE ORANGUTAN WITH ID 4 ONTO TILE 1;`
`MOVE ORANGUTAN WITH ID 4 ONTO TILE 3;`
`USE WARDROBE WITH ID 7 BY ORANGUTAN WITH ID 5;`
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`TILE ID = 2 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`TILE ID = 3 CURRENTANIMAL = (SLEEPY PANDA 6) CURRENTITEM = (WARDROBE 7)`
`TILE ID = 4 CURRENTANIMAL = (ORANGUTAN 5) CURRENTITEM = (WARDROBE 8)`

[ANIMALS]

`ORANGUTAN ID = 5 STANDINGON = (TILE 4) LEADERANIMAL = (NULL) GUIDEDANUMAL = (SLEEPY PANDA 6) THEFTTIMER = 0`
`SLEEPY PANDA ID = 6 STANDINGON = (TILE 3) SLEEPINGIN = (NULL) LEADERANIMAL = (ORANGUTAN 5) GUIDEDANUMAL = (NULL)`

[ITEMS]

`WARDROBE ID = 7 PLACEDON = (TILE 3)`
`WARDROBE ID = 8 PLACEDON = (TILE 4)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2`
`TILE 2 CONNECTED WITH TILE 3`
`WARDROBE 7 CONNECTED WITH WARDROBE 8`

## 13. Panda uses wardrobe

#### Bemenet:

`ADD TILE WITH ID 1;`
`ADD TILE WITH ID 2;`
`ADD TILE WITH ID 3;`
`ADD SLEEPY PANDA WITH ID 3 ONTO TILE 1;`
`ADD WARDROBE WITH ID 4 ONTO TILE 2;`
`ADD WARDROBE WITH ID 5 ONTO TILE 3;`

`CONNECT TILE 1 WITH TILE 2;`
`CONNECT WARDROBE 4 WITH WARDROBE 5;`

`BEGIN;`
`MOVE SLEEPY PANDA WITH ID 3 ONTO TILE 2;`
`USE WARDROBE WITH ID 4 BY SLEEPY PANDA WITH ID 3;`
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`TILE ID = 2 CURRENTANIMAL = (NULL) CURRENTITEM = (WARDROBE 4)`
`TILE ID = 3 CURRENTANIMAL = (SLEEPY PANDA 3) CURRENTITEM = (WARDROBE 5)`

[ANIMALS]

`SLEEPY PANDA ID = 3 STANDINGON = (TILE 3) SLEEPINGIN = (NULL) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`

[ITEMS]

`WARDROBE ID = 4 PLACEDON = (TILE 2)`
`WARDROBE ID = 5 PLACEDON = (TILE 3)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2`
`WARDROBE 4 CONNECTED WITH WARDROBE 5`

## 14. Panda sleeps alone

#### Bemenet:

`ADD TILE WITH ID 1;`
`ADD TILE WITH ID 2;`
`ADD TILE WITH ID 3;`
`ADD SLEEPY PANDA WITH ID 4 ONTO TILE 1;`
`ADD COUCH WITH ID 5 ONTO TILE 3;`

`CONNECT TILE 1 WITH TILE 2;`

`BEGIN;`
`MOVE SLEEPY PANDA WITH ID 3 ONTO TILE 2;`
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`TILE ID = 2 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`TILE ID = 3 CURRENTANIMAL = (SLEEPY PANDA 3) CURRENTITEM = (COUCH 5)`

[ANIMALS]

`SLEEPY PANDA ID = 3 STANDINGON = (TILE 3) SLEEPINGIN = (COUCH 5) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`

[ITEMS]

`COUCH ID = 5 PLACEDON = (TILE 3)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2`
`TILE 2 CONNECTED WITH TILE 3`

## 15. Panda sleeps in line

#### Bemenet:

`/load sleep_test.txt`

`BEGIN;`
`MOVE ORANGUTAN WITH ID 7 ONTO TILE 4;`
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`TILE ID = 2 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`TILE ID = 3 CURRENTANIMAL = (COWARD PANDA 9) CURRENTITEM = (NULL)`
`TILE ID = 4 CURRENTANIMAL = (ORANGUTAN 7) CURRENTITEM = (NULL)`
`TILE ID = 5 CURRENTANIMAL = (SLEEPY PANDA 8) CURRENTITEM = (COUCH 10)`

[ANIMALS]

`COWARD PANDA ID = 3 STANDINGON = (TILE 2) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`
`SLEEPY PANDA ID = 3 STANDINGON = (TILE 5) SLEEPINGIN = (COUCH 10) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`
`ORANGUTAN ID = 3 STANDINGON = (TILE 4) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL) THEFTTIMER = 0`

[ITEMS]

`COUCH ID = 10 PLACEDON = (TILE 5)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2`
`TILE 2 CONNECTED WITH TILE 3`
`TILE 3 CONNECTED WITH TILE 4`
`TILE 3 CONNECTED WITH TILE 5`

## 16. Panda gets scared alone

#### Bemenet:
`ADD TILE WITH ID 1;`
`ADD TILE WITH ID 2;`
`ADD VENDINGMACHINE WITH ID 3 ONTO TILE 1;`
`ADD COWARD PANDA WITH ID 4 ONTO TILE 2;`

`CONNECT TILE 1 WITH TILE 2;`

`BEGIN;`
`WORK VENDINGMACHINE WITH ID 3;`
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (VENDINGMACHINE 3)`
`TILE ID = 2 CURRENTANIMAL = (COWARD PANDA 4) CURRENTITEM = (NULL)`

[ANIMALS]

`COWARD PANDA ID = 4 STANDINGON = (TILE 2) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`

[ITEMS]

`VENDINGMACHINE ID = 3 PLACEDON = (TILE 1)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2;`

## 17. Panda gets scared in line

#### Bemenet:
`/load scare_test.txt`

`BEGIN;`
`MOVE ORANGUTAN WITH ID 7 ONTO TILE 4;`
`WORK VENDINGMACHINE WITH ID 8;`
`END;`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`TILE ID = 2 CURRENTANIMAL = (SLEEPY PANDA 6) CURRENTITEM = (NULL)`
`TILE ID = 3 CURRENTANIMAL = (COWARD PANDA 7) CURRENTITEM = (NULL)`
`TILE ID = 4 CURRENTANIMAL = (ORANGUTAN 8) CURRENTITEM = (NULL)`
`TILE ID = 5 CURRENTANIMAL = (NULL) CURRENTITEM = (VENDINGMACHINE 9)`

[ANIMALS]

`COWARD PANDA ID = 7 STANDINGON = (TILE 3) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`
`SLEEPY PANDA ID = 6 STANDINGON = (TILE 2) SLEEPINGIN = (NULL) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`
`ORANGUTAN ID = 8 STANDINGON = (TILE 4) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL) THEFTTIMER = 0`

[ITEMS]

`VENDINGMACHINE ID = 9 PLACEDON = (TILE 5)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2`
`TILE 2 CONNECTED WITH TILE 3`
`TILE 3 CONNECTED WITH TILE 4`
`TILE 3 CONNECTED WITH TILE 5`


## 18. Panda jumps alone on safe tile

#### Bemenet:

`ADD TILE WITH ID 1;`
`ADD TILE WITH ID 2;`
`ADD GAMEMACHINE WITH ID 3 ONTO TILE 1;`
`ADD JUMPY PANDA WITH ID 4 ONTO TILE 2;`

`CONNECT TILE 1 WITH TILE 2;`

`BEGIN;`
`WORK GAMEMACHINE WITH ID 3;`
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (GAMEMACHINE 3)`
`TILE ID = 2 CURRENTANIMAL = (JUMPY PANDA 4) CURRENTITEM = (NULL)`

[ANIMALS]

`JUMPY PANDA ID = 4 STANDINGON = (TILE 2) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`

[ITEMS]

`GAMEMACHINE ID = 3 PLACEDON = (TILE 1)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2;`

## 19. Panda jumps in line on safe tile

#### Bemenet:

`/load jumpy_safe_test.txt`

`BEGIN;`
`MOVE ORANGUTAN WITH ID 8 ONTO TILE 4;`
`WORK GAMEMACHINE WITH ID 9;`
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`TILE ID = 2 CURRENTANIMAL = (COWARD PANDA 6) CURRENTITEM = (NULL)`
`TILE ID = 3 CURRENTANIMAL = (JUMPY PANDA 7) CURRENTITEM = (NULL)`
`TILE ID = 4 CURRENTANIMAL = (ORANGUTAN 8) CURRENTITEM = (NULL)`
`TILE ID = 5 CURRENTANIMAL = (NULL) CURRENTITEM = (GAMEMACHINE 9)`

[ANIMALS]

`COWARD PANDA ID = 6 STANDINGON = (TILE 3) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`
`JUMPY PANDA ID = 7 STANDINGON = (TILE 2) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`
`ORANGUTAN ID = 8 STANDINGON = (TILE 4) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL) THEFTTIMER = 0`

[ITEMS]

`GAMEMACHINE ID = 9 PLACEDON = (TILE 5)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2`
`TILE 2 CONNECTED WITH TILE 3`
`TILE 3 CONNECTED WITH TILE 4`
`TILE 3 CONNECTED WITH TILE 5`

## 20. Panda jumps alone on fragile tile

#### Bemenet:

`ADD TILE WITH ID 1;`
`ADD FRAGILE TILE WITH ID 2 WITH LIFE 5;`
`ADD GAMEMACHINE WITH ID 3 ONTO TILE 1;`
`ADD JUMPY PANDA WITH ID 4 ONTO TILE 2;`

`CONNECT TILE 1 WITH TILE 2;`

`BEGIN;`
`WORK GAMEMACHINE WITH ID 3;`
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (VENDINGMACHINE 3)`
`FRAGILE TILE ID = 2 LIFE = 4 CURRENTANIMAL = (JUMPY PANDA 4) CURRENTITEM = (NULL)`

[ANIMALS]

`JUMPY PANDA ID = 4 STANDINGON = (TILE 2) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`

[ITEMS]

`GAMEMACHINE ID = 3 PLACEDON = (TILE 1)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2;`

## 21. Panda jumps in line on fragile tile

#### Bemenet:

`/load jumpy_fragile_test.txt`

`BEGIN;`
`MOVE ORANGUTAN WITH ID 7 ONTO TILE 4;`
`WORK GAMEMACHINE WITH ID 8;`
`END;`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`TILE ID = 2 CURRENTANIMAL = (COWARD PANDA 6) CURRENTITEM = (NULL)`
`FRAGILE TILE ID = 3 LIFE = 4 CURRENTANIMAL = (JUMPY PANDA 7) CURRENTITEM = (NULL)`
`TILE ID = 4 CURRENTANIMAL = (ORANGUTAN 8) CURRENTITEM = (NULL)`
`TILE ID = 5 CURRENTANIMAL = (NULL) CURRENTITEM = (GAMEMACHINE 9)`

[ANIMALS]

`COWARD PANDA ID = 6 STANDINGON = (TILE 3) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`
`JUMPY PANDA ID = 7 STANDINGON = (TILE 2) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`
`ORANGUTAN ID = 8 STANDINGON = (TILE 4) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL) THEFTTIMER = 0`

[ITEMS]

`GAMEMACHINE ID = 9 PLACEDON = (TILE 5)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2`
`TILE 2 CONNECTED WITH TILE 3`
`TILE 3 CONNECTED WITH TILE 4`
`TILE 3 CONNECTED WITH TILE 5`

## 22. Panda jumps alone on broken tile

#### Bemenet:

`ADD TILE WITH ID 1;`
`ADD FRAGILE TILE WITH ID 2 WITH LIFE 1;`
`ADD GAMEMACHINE WITH ID 3 ONTO TILE 1;`
`ADD JUMPY PANDA WITH ID 4 ONTO TILE 2;`

`CONNECT TILE 1 WITH TILE 2;`

`BEGIN;`
`WORK GAMEMACHINE WITH ID 3;`
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (GAMEMACHINE 3)`
`BROKEN TILE ID = 2 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2;`


## 23. Panda jumps in line on broken tile

#### Bemenet:

`/load jumpy_broken_test.txt`

`BEGIN;`
`MOVE ORANGUTAN WITH ID 7 ONTO TILE 4;`
`WORK GAMEMACHINE WITH ID 8;`
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`TILE ID = 2 CURRENTANIMAL = (COWARD PANDA 6) CURRENTITEM = (NULL)`
`BROKEN TILE ID = 3 CURRENTANIMAL = (NULL) CURRENTITEM = (NULL)`
`TILE ID = 4 CURRENTANIMAL = (ORANGUTAN 8) CURRENTITEM = (NULL)`
`TILE ID = 5 CURRENTANIMAL = (NULL) CURRENTITEM = (GAMEMACHINE 9)`

[ANIMALS]

`COWARD PANDA ID = 6 STANDINGON = (TILE 3) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`
`ORANGUTAN ID = 8 STANDINGON = (TILE 4) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL) THEFTTIMER = 0`

[ITEMS]

`GAMEMACHINE ID = 9 PLACEDON = (TILE 5)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2`
`TILE 2 CONNECTED WITH TILE 3`
`TILE 3 CONNECTED WITH TILE 4`
`TILE 3 CONNECTED WITH TILE 5`

## 24. Orangutan lets go

#### Bemenet:

`ADD TILE WITH ID 1;`
`ADD TILE WITH ID 2;`
`ADD SLEEPY PANDA WITH ID 3 ONTO TILE 1;`
`ADD ORANGUTAN WITH ID 4 ONTO TILE 2;`

`CONNECT TILE 1 WITH TILE 2;`

`BEGIN;`
`MOVE ORANGUTAN WITH ID 4 ONTO TILE 1;`
`RELEASE ORANGUTAN WITH ID 4;`
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (SLEEPY PANDA 3) CURRENTITEM = (NULL)`
`TILE ID = 2 CURRENTANIMAL = (ORANGUTAN 4) CURRENTITEM = (NULL)`

[ANIMALS]

`SLEEPY PANDA ID = 3 STANDINGON = (TILE 1) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`
`ORANGUTAN ID = 4 STANDINGON = (TILE 2) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL)`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2;`

## 25. Orangutan steals from orangutan

#### Bemenet:

`ADD TILE WITH ID 1;`
`ADD TILE WITH ID 2;`
`ADD TILE WITH ID 3;`
`ADD SLEEPY PANDA WITH ID 4 ONTO TILE 1;`
`ADD ORANGUTAN WITH ID 5 ONTO TILE 2;`
`ADD ORANGUTAN WITH ID 6 ONTO TILE 3;`

`CONNECT TILE 1 WITH TILE 2;`
`CONNECT TILE 2 WITH TILE 3;`

`BEGIN;`
`MOVE ORANGUTAN WITH ID 5 ONTO TILE 1;`
`MOVE ORANGUTAN WITH ID 6 ONTO TILE 2;`
`END;`

`/print`

#### Elvárt kimenet:

[TILES]

`TILE ID = 1 CURRENTANIMAL = (SLEEPY PANDA 4) CURRENTITEM = (NULL)`
`TILE ID = 2 CURRENTANIMAL = (ORANGUTAN 6) CURRENTITEM = (NULL)`
`TILE ID = 3 CURRENTANIMAL = (ORANGUTAN 5) CURRENTITEM = (NULL)`

[ANIMALS]

`SLEEPY PANDA ID = 4 STANDINGON = (TILE 1) LEADERANIMAL = (ORANGUTAN 6) GUIDEDANUMAL = (NULL)`
`ORANGUTAN ID = 5 STANDINGON = (TILE 3) LEADERANIMAL = (NULL) GUIDEDANUMAL = (NULL) THEFTTIMER = 3`
`ORANGUTAN ID = 6 STANDINGON = (TILE 2) LEADERANIMAL = (NULL) GUIDEDANUMAL = (SLEEPY PANDA 4) THEFTTIMER = 0`

[CONNECTIONS]

`TILE 1 CONNECTED WITH TILE 2`
`TILE 2 CONNECTED WITH TILE 3`
